...
import { URL } from 'url';
import { redisSubscriber } from './database';
...
const requestListener = (
  ...
  if (req.headers.accept && req.headers.accept === 'text/event-stream') {
    ...
    if (req.url === '/health') {
      ...
      res.writeHead(200, { 'Content-Type': 'application/json' });
      const worksetStats: { [key: string]: number } = {};
...
const handleSSE = (req: http.IncomingMessage, res: http.ServerResponse) => {
...
const extractWorkSetIdFromChannel = (channel: string): string | null => {
