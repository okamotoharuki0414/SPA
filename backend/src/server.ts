...
import { pgPool, redisPublisher, connectDatabases } from './database';
...
app.get('/api/shift/:worksetId', async (req, res) => {
  try {
    ...
    const result = await pgPool.query(
      'SELECT * FROM shift WHERE workset_id = $1 ORDER BY shift_date, employee_id',
      [worksetId]
    );
...
app.post('/api/shift', async (req, res) => {
  try {
    ...
    const result = await pgPool.query(
      `INSERT INTO shift (workset_id, employee_id, employee_name, shift_date, start_time, end_time, kinmu_pattern_id, work_hours, notes, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, CURRENT_TIMESTAMP)
       RETURNING *`,
      [workset_id, employee_id, employee_name, shift_date, start_time, end_time, kinmu_pattern_id, work_hours, notes]
    );
...
app.put('/api/shift/:shiftId', async (req, res) => {
  try {
    ...
    const result = await pgPool.query(
      `UPDATE shift SET employee_name = $1, shift_date = $2, start_time = $3, end_time = $4,
       kinmu_pattern_id = $5, work_hours = $6, notes = $7, updated_at = CURRENT_TIMESTAMP
       WHERE shift_id = $8 RETURNING *`,
      [employee_name, shift_date, start_time, end_time, kinmu_pattern_id, work_hours, notes, shiftId]
    );
...
app.delete('/api/shift/:shiftId', async (req, res) => {
  try {
    const shiftId = parseInt(req.params.shiftId);
    await pgPool.query('DELETE FROM shift WHERE shift_id = $1', [shiftId]);
    res.status(204).send();
  } catch (error) {
...
app.get('/api/kinmupattern/:worksetId', async (req, res) => {
  try {
    const worksetId = parseInt(req.params.worksetId);
    const result = await pgPool.query(
      'SELECT * FROM kinmupattern WHERE workset_id = $1 AND is_active = true ORDER BY display_order',
      [worksetId]
    );
    res.json(result.rows);
  } catch (error) {
...
const listenToNotifications = async () => {
  ...
  client.on('notification', (msg) => {
    ...
    if (payload) {
      ...
      redisPublisher.publish(channel, payload).catch(console.error);
    }
  });
